...
import { docsUrl, type Finding, type ItemType } from "./types.js";

export function metadataSchemaFindings(metadata: unknown, itemType: ItemType): Finding[] {
  ...
  const add = (where: string, message: string, measured?: string, limit?: string) => {
    ...
    findings.push({
      ...
      docs: docsUrl("metadata"), where, measured, limit,
      message
    });
  };
