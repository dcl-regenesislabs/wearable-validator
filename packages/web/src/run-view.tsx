...
import { GROUP_LABELS, isRunning, renderingGroupShown, visualRows, visualStepsOf, type Review, type VisualPhase, type VisualState } from "./progress.js";
import { PendingCaptures, Stepper } from "./stepper.js";
import { FindingCard, RuleColumns, SectionToggle, Spinner, StatusChip, filterStatuses, headToggle } from "./rules.js";
import type { Collapsed } from "./sections.js";
...
/** The collapse state of the result sections, owned by the results page; this view's keys are "visual" and "rendering". */
export interface Sections {
  /** Prefix of the element ids the toggles point at, unique per mounted results page. */
  scope: string;
  collapsed: Collapsed;
...
export interface RunViewProps {
  ...
  modelKnown: boolean;
  sections: Sections;
  /** Result filter key from the results toolbar; absent shows every row. */
  filter?: string;
  onCancel?: () => void;
  onRunAgain?: () => void;
...
export function RunView({ state, aiChecks, modelKnown, sections, filter, onCancel, onRunAgain, gate }: RunViewProps) {
  const running = isRunning(state.phase);
  const steps = visualStepsOf(state, aiChecks);
  const rows = visualRows(state);
  const visualBody = `${sections.scope}-visual-body`;
  const visualCollapsed = sections.collapsed.has("visual");
  return (
    <>
      <section className={`eui-panel visual-card ${state.phase}`}>
        <div className="eui-panel-head collapsible" onClick={headToggle(() => sections.toggle("visual"))}>
          <div className="eui-head-text">
            <span className="eui-overline">Visual review</span>
            <span className="eui-title">{state.phase === "uploading" && state.reference ? "Fetching from the catalyst" : PHASE_WORDS[state.phase]}</span>
          </div>
          <div className="eui-head-actions">
            ...
            <SectionToggle name="Visual review" controls={visualBody} collapsed={visualCollapsed} onToggle={() => sections.toggle("visual")} />
          </div>
        </div>
        <div className="eui-panel-body visual-body" id={visualBody} hidden={visualCollapsed}>
          {state.phase !== "idle" ? (
      ...
      {renderingGroupShown(state) && <RenderingGroup state={state} rows={rows} modelKnown={modelKnown} sections={sections} filter={filter} />}
    </>
  );
}

function RenderingGroup({ state, rows, modelKnown, sections, filter }: { state: VisualState; rows: ReturnType<typeof visualRows>; modelKnown: boolean; sections: Sections; filter?: string }) {
  ...
  const body = `${sections.scope}-rendering-body`;
  const collapsed = sections.collapsed.has("rendering");
  return (
    <section ref={root} className="group rendering" aria-busy={rendering || undefined}>
      <div className="group-head collapsible" onClick={headToggle(() => sections.toggle("rendering"))}>
        ...
        <SectionToggle name={GROUP_LABELS.rendering} controls={body} collapsed={collapsed} onToggle={() => sections.toggle("rendering")} />
      </div>
