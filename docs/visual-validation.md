...
**Status.** V-05 `thumbnail-honesty` is built and runnable; V-01..V-04, V-06 and V-07 are not. The rule is one file (`packages/wearable-validator/src/checks/rendering/thumbnail-honesty/index.ts`); the renderer and the vision call are one optional entry each (`src/adapters/rendering.ts`, `src/adapters/ai.ts`); the shared evidence helper is `src/logic/captures.ts`; the local runner is `tools/src/visual-review.ts`. Every run writes a folder you can open (§3). Proof on file (2026-09-11, local PR #10053 build, SwiftShader): the bundled shirt's six worn views are byte-identical across every run and across the previous implementation; the six isolated-item views vary in framing between runs (renderer camera fit, see the gap below); `--no-ai` writes the prompt without spend; `--from <run> --answer` replays a saved answer with zero network; one real Sonnet 4.5 call passed the matching thumbnail ($0.074) and flagged a foreign one ($0.071). `manifest.version` moved to 0.2.0 with the new rule.

**Known renderer gap.** In item-only mode (`type: "wearable"`) the wrapper's automatic camera fit is timing-dependent: three consecutive runs produced two different framings for the same body shape while the worn views never changed. The prompt tolerates camera differences, so verdicts held, but capture reuse across runs cannot promise identical isolated pixels until the fit is made deterministic (a fixed camera distance after the pose settles, or an explicit `changeZoom`/`panCamera` reset).
