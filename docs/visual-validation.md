...
`ANTHROPIC_OAUTH_SETUP_TOKEN=<token> npm start -w wearable-validator-server` starts a local run server on `127.0.0.1:4180` (`packages/server/src/main.ts` → `server.ts`); `npm run serve` at the root does the same after building the site into it. The Vite dev server proxies `/api` to it, so the site gains a **Visual review** panel under the code results: when the code checks pass the site uploads the zip on its own (with errors it waits for **Render and review anyway**, so no screenshot is taken for an item that needs fixing first), and the panel shows the server-side code gate, each screenshot the moment it is captured, the prompt version with a link to the exact prompt and image order, the raw answer with token usage, and the resulting findings as rule rows in the same table design as the code checks, with evidence chips that highlight the capture they cite. Under the panel header, **Your runs** lists the caller's earlier runs (name, relative time, status); clicking one replays it instead of uploading. Locally the owner is `local`; hosted, the header shows **Signed in as <email>**. The public production site never shows the panel: its Worker answers `/api/*` with a 404 because no `API_ORIGIN` is configured ([deployment.md](deployment.md)).
...
| `POST /api/runs[?standalone=1][&model=0]` (zip bytes, `content-type: application/zip`, URL-encoded `x-file-name`) | `201 { id }`; 415 without that content type (a form or no-cors fetch from another site cannot send it); 400 when the name does not decode; 409 while another run is active; the code gate runs first and, when it fails, stops before any screenshot unless `standalone`; `model=0` renders only |
...
|---|---|---|---|
| 0 | the folder | `packages/server/src/review.ts` · `main()` (the server: `server.ts` per `POST /api/runs`) | Code gate passes → `createRenderer({ buildDirectory })`, `createPiReviewer({ credentials })` wrapped in `recordingReviewer` → `validate(zip, { checks: ["thumbnail-honesty"], captures, services, signal })` |
...
| 8 | `result.json`, `index.html` | `src/validate.ts` · `normalizeExecution()`, `computePassed()`; `packages/server/src/runs.ts` · `writeRun()` | Cross-checks status vs findings, `passed: null` (subset), `Result.captures` and `checks[0].review` populated; the CLI serializes |
...
| 11 | `packages/server/src/review.ts` + `runs.ts` + `reviewers.ts` | ~300 | `review.ts`: `readArgs()` · `main()` guarded by `process.argv[1] === fileURLToPath(import.meta.url)` · `runs.ts`: `readEvidenceFile(path)` (refuses `.env*`) · `readRun(dir)` · `writeRun(dir, result, thumbnail)` · `promptMarkdown(request)` · `galleryHtml(result)` · `reviewers.ts`: `tokenCredentials(token): CredentialStore` (in-memory setup token, refuses non-`sk-ant-oat`) · `recordingReviewer(reviewer, dir)` · `dryRunReviewer()` · `replayReviewer(answerPath)` · `liveReviewer(reviewer, run)` | The wiring points: `createRenderer`/`createPiReviewer` are constructed only in `review.ts` and `main.ts`. Also the only place the boundary is recorded — the package stays unaware. |
...
| 13 | `packages/server/test/runs.test.ts` (+ `server.test.ts` for the routes) | ~110 | — | Child-process gate verbatim; `tokenCredentials` seeds a year-long OAuth credential and refuses API keys; `writeRun → readRun` round-trips captures byte-for-byte in a tmp dir. |
...
Written by `packages/server/src/runs.ts` (`writeRun`) for both the CLI and the server. Gitignored under `packages/server/artifacts/` (`ARTIFACTS_DIR` or `--out` elsewhere). Server runs add `input.json` (`{ id, owner, name, startedAt, sha256 }`) and `events.jsonl`.
...
| OAuth credential | `packages/server/src/reviewers.ts` · `tokenCredentials()` | `// a claude setup-token lives about a year and is itself the bearer, not a refresh token` |
| Local runner flow, gate, exit codes, SIGINT, `stop()` in finally | `packages/server/src/review.ts` · `main()` | `// code gate is zero-cost: no browser, no OAuth until passed === true or --standalone` |
...
- The run API knows curators only (Cloudflare Access email). ADR-44 signed fetch for the Builder (owner = wallet) is the next `Identify` provider in `packages/server/src/identity.ts`. Also open: a queue instead of the 409 busy answer, run retention, a daily spend cap ([deployment.md](deployment.md)).
