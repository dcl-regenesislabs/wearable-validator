...
| `GET /api/runs/:id/events` | Server-Sent Events: `check` (start/finish of every check), `gate` (code result), `queue` (`{ position, ahead, running, averageRunMs, etaMs }` every time the line moves; position 0 = your turn), `stage` (`{ text }`; the rendering stage also carries `views`, the number of `capture` events to expect, and `bodyShapes`), `capture` (`{ id, request, url }` as each PNG lands), `review` (`request` with prompt digest and image order, then `answer` = the `ReviewResult`), `done` (the `Result` with capture URLs) or `error`. Events carry ids; `Last-Event-ID` replays the rest; a finished run loaded from disk replays as one `done` event |
