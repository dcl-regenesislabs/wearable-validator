...
`packages/wearable-validator` (published package: 35 deterministic checks, manifest, CLI) ·
...
- **Registry-driven surfaces.** Each check must have entries in `explanations.ts` (plain language), `fixes.ts` (concrete steps), `details.ts` (how it's checked), `docs-links.ts` (exact-section docs anchor) — completeness is test-enforced.
...
ESM (`type: module`, `.js`-suffixed relative imports, `node:` builtins), strict minimal tsconfig, kebab-case flat modules, no barrels, `interface` + string-literal unions, plain `new Error("actionable sentence")`, node:test with flat `test/X.test.ts`, exact-pin risky deps with a why-comment. The website uses dcl-editor's design tokens verbatim (see packages/debug-ui/src/styles.css header).
