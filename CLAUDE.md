...
- `npm run dev -w wearable-validator-debug-ui` — the website (Vite); with `npm run serve -w wearable-validator-tools -- --renderer-build <Build> [--auth .auth.json]` running, the site streams visual reviews live over SSE
